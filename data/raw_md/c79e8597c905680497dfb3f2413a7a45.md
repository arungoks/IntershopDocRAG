---
id: 'c79e8597c905680497dfb3f2413a7a45'
title: 'Guide'
url: 'https://knowledge.intershop.com/kb/index.php/Display/2C8089'
scraped_at: '2026-04-21T05:39:42.518214+00:00'
---
...
User budgets and order spend limits can be defined separately for each user. If defined, these values are used to decide if an order requires approval or not. An approval is necessary if one or both values are exceeded. If no values are defined, orders can be placed without approval. For more information please read the section [Order Approval](https://knowledge.intershop.com#Guide-B2BStorefrontFunctionality(validto7.10.16)-OrderApproval).

There are jobs which are created with the purpose to reset the user budgets at the end/start of each period. The following budget types are available: monthly, quarterly and weekly. The names of the corresponding job configurations are: ResetMonthlyBudgetsSpentJob, ResetQuarterlyBudgetsSpentJob and ResetWeeklyBudgetsSpentJob. They can be found in the Intershop System Management's *Schedules | Scheduling* menu item under root domain:
...
This way resetting of user budgets will be performed automatically at the desired time. It is not necessary to configure all three jobs if for example only one type of budget period is used. For example, if weekly is the only one available, then ResetMonthlyBudgetsSpentJob and ResetQuarterlyBudgetsSpentJob can remain disabled.
...
B2B customer organizations can use order approval to check orders before they are placed. The approval contains two checks, one for [cost center specific approval](https://knowledge.intershop.com#Guide-B2BStorefrontFunctionality(validto7.10.16)-CostCenterApproval) and one to check orders against a users budget and order spend limit. Using the order approval is optional. If no budgets and spend limits are defined for users and no cost centers are defined for a user, orders are placed without approval.
...
Using the CSV import, you can import a customizable number of products in one step, just by providing a CSV file. Find a sample CSV import file in the following zip file: [quickorder.zip](https://knowledge.intershop.com/kb/index.php/Deliver/DOC/2C8089/quickorder.zip)
