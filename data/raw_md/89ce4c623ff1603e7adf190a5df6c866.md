---
id: '89ce4c623ff1603e7adf190a5df6c866'
title: 'Concept'
url: 'https://knowledge.intershop.com/kb/index.php/Display/4D7862'
scraped_at: '2026-04-21T05:29:54.526514+00:00'
---
This concept introduces the recurring order feature, available through the Recurring Order Extension, see [Public Release Note - Recurring Order Extension 1](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/49213604027/Public+Release+Note+-+Recurring+Order+Extension+1). Developers will get an overview of the feature's functionality, key components, and implementation details.

|
|
|---|---|
Recurring Order | A recurring order refers to a regular purchase that happens at a set frequency (e.g., weekly, monthly) and typically requires manual intervention by the customer. Customers may need to confirm or reorder at these intervals by adding items to their cart or authorizing payment. While a company might automatically generate the order based on the set schedule, the customer retains control over each transaction. Example: Setting up a recurring order for monthly coffee delivery requires the customer to approve or pay for each subsequent order as it becomes due. This concept is often compared to subscriptions, however, there are differences in terms of automation, customer involvement, and payment processes, see S |
...
The scheduling of recurring orders is done by a scheduling job that is provided by the [Recurring Order Extension](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/49213604027/Public+Release+Note+-+Recurring+Order+Extension+1). See [Scheduling Service Job](https://knowledge.intershop.com#ImplementationSchedulingServiceJob) for further details.
...
The *Recurring Order S*ervice includes functions to:
...
It can be found in the SMC *Scheduling *view. For details about the job, see the job description in [Job - Recurring Order Schedule Service (Recurring Order Extension)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/49409687553/Job+-+Recurring+Order+Schedule+Service+Recurring+Order+Extension).

The start and end dates of a recurring order do not contain time information. The schedule (run time) of the *Recurring Order Scheduling Service* job defines when the recurring orders are triggered. If the job runs, for example, every day in the morning at 09.00 AM, all orders that are due or overdue on that date will be generated.
