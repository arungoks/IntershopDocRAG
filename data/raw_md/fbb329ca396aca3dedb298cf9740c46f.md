---
id: 'fbb329ca396aca3dedb298cf9740c46f'
title: 'Cookbook'
url: 'https://knowledge.intershop.com/kb/index.php/Display/A25599'
scraped_at: '2026-04-21T05:39:53.280793+00:00'
---
This cookbook gives practical advise on how to develop using Gradle and Intershop build and deployment tools. The target audience are developers of Intershop customer projects and partners. Knowledge of [Concept - Continuous Delivery Tools (valid to 7.10)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1784317189) is assumed.The cookbook presents a directory setup and workflow, that is optimized for a single system component set and assembly.


Names in angle brackets

Upper case names in angle brackets like *<COMPONENTSET>* are references to environment variables. Replace them by the proper OS-dependent syntax, e.g., `%COMPONENTSET%`

on Windows or `$COMPONENTSET`

on Linux. The environment variables are set by the *gradle_environment.bat* or *gradle_environment.sh* scripts (see recipes [Setup a Development Environment](https://knowledge.intershop.com#Cookbook-GradleDeveloperWorkflow(validtoGradleTools2.7)-SetupDevelopmentEnvironment) and [Run Gradle Tasks](https://knowledge.intershop.com#Cookbook-GradleDeveloperWorkflow(validtoGradleTools2.7)-RunGradleTasks)).
...
[Cookbook - Gradle Assembly Tools (valid to 7.8)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1788155636) - create an *assembly*
...
Ensure the [preconditions](https://knowledge.intershop.com#Cookbook-GradleDeveloperWorkflow(validtoGradleTools2.7)-Preconditions) are fulfilled before starting the development.
...
| 2 |
Create an empty directory to contain your development environment. Check out preconfigured Gradle user home ( | The partition/drive should have enough space/speed to hold your sources, a deployed server and the size of a deployed server caches for remote repositories.
|
| 2 |
Create an empty directory to contain your development environment. Check out the developer home from your VCS to the subdirectory | The partition/drive should have enough space/speed to hold your sources, a deployed server and the size of a deployed server caches for remote repositories. |
| 3 | Open a console, change into the checked out folder and execute | This sets several environment variables, including `GRADLE_USER_HOME` , `COMPONENTSET` , `ASSEMBLY, SERVER, REPO` inside this console. We will refer to this console as developer console. Keep using it through the remaining steps. |
| 4 | Check out your system component set to the folder pointed to by | |
| 5 | Check out your assembly to the folder pointed to by `%ASSEMBLY%` using your VCS. | |
| 6 | Go to | This builds your system component sets. The results can be found at The |
| 7 | Go to | This creates the file <ASSEMBLY>\environment.properties.sample. It is a template for all deployment configuration required to deploy a server for development. |
| 8 | Copy | At least adjust the following sections: |
| 9 | Go to <ASSEMBLY> (run `cdAssembly` )* and run `gradlew deployServer` | This deploys the assembly as your test server into |
| 10 | Run as adminstrator/with root privileges (as requested in the console output): | This creates the services. The services run under your user. You will be prompted for your password. |
| 11 | Go to
| See the |
| 12 | At the developer console go to your Intershop Studio installation and run (or run | It is important to start Intershop Studio from a developer console with all environment variables set. No more initial configuration in the preference menu is necessary. Using Gradle requires Intershop Studio version >= 4.3.4.
|
| 13 | Import the system component set using the wizard Component Set projects into Workspace | See the |
...
| 5 | Check out your assembly to the folder pointed to by `$ASSEMBLY` using your VCS. | |
| 6 | Go to | This builds your system component sets. The results can be found at The |
| 7 | Go to <ASSEMBLY> (run `cdAssembly` )* and run `./gradlew` | This creates the file <ASSEMBLY>/environment.properties.sample. It is a template for all deployment configuration required to deploy a server for development. |
| 8 | Copy | At least adjust the following sections: |
| 9 | Go to <ASSEMBLY>(run `cdAssembly` )* and run `./gradlew deployServer` | This deploys the assembly as your test server into |
| 10 | Run as adminstrator/with root privileges (as requested in the console output): | This creates the init.d-scripts. The services will run under your user. |
| 11 | Go to
| See the |
| 12 | At the developer console go to your Intershop Studio installation and run (or run | It is important to start Intershop Studio from a developer console with all environment variables set. No more initial configuration in the preference menu is necessary. Using Gradle requires Intershop Studio version >= 4.3.4.
|
...
|---|---|---|
| Gradle user home | Developer configuration in gradle.properties, as well as caches for downloads from remote repositories | `GRADLE_USER_HOME` |
| System Component set directory | VCS working copy of component set sources | `COMPONENTSET` |
| Assembly directory | VCS working copy of assembly sources | `ASSEMBLY` |
| Server directory | Server deployed for manual and automatic testing (contains both IS_HOME and as subfolders)`IS_SHARE` | `SERVER` |
| Javadoc directory | Javadoc of cartridges (excluding own cartridges) | `JAVADOC` |
| Intershop Studio workspace | Workspace data | `WORKSPACE` |
| Local repository | Locally published components – used internally as a deployment source | `REPO` |
...
| Command | Function |
|---|---|
`cdRepo` | Change directory to local repository |
`cdSet` | Change directory to project component set |
`cdAssembly` | Change directory to project assembly |
`cdServer` | Change directory to server directory |
`cdDoc` | Change directory to java doc directory |
...
. (Project dependencies resolved directly from the target directories.)The `useSetAsFilter`

task of the component set adds a filter property in the *<GRADLE_USER_HOME>/gradle.properties*:
...
The developer deployment first generates a *<SERVER>/settings.gradle* file from the *environment.properties* configuration. Based on this file it then runs standard deployment procedure as describes in [Concept - Gradle Deployment Tools](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1775684692/Concept+-+Gradle+Deployment+Tools) specifying the host type `all`

, e.g., deploying all host types of the assembly. As deployment source it uses the repositories declared in your corporate plugin as well as your local repository at *<REPO>*.
...
After the post-install scripts, running services / applications is completely up to you and works mostly as before. If starting any service fails on Windows, please refer to the Recipe [(Windows) Starting a Service Fails](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1783302208#Cookbook-GradleDeploymentTools(7.4CI-ICM7.7)-StartingServiceFails) in the [Cookbook - Gradle Deployment Tools (7.4 CI - ICM 7.7)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1783302208)
...
To execute a task in all cartridges of the component set execute `gradlew <task>`

(without colon), e.g., `gradlew build`

. Running the command with a colon, e.g., `gradlew :build`

, leads to an error if the task does only exist for cartridges, but not for the component set:

FAILURE: Could not determine which tasks to execute. * What went wrong: Task 'build' not found in root project 'projectComponentSet'. * Try: Run gradlew tasks to get a list of available tasks.
...
in *<GRADLE_USER_HOME>/gradle.properties*. This way rerunning the `importDump`
...
The changed code artifact resides in <COMPONENTSET>/<cartridge>/staticfiles/share. | Publish and redeploy the affected cartridge(s) and restart the application server / rerun the dbinit. (Publishing automatically includes rebuilding.) |
| You are unsure which kinds of changes have been made - e.g., you updated your VCS working copy. | Publish and redeploy the component set. You may also want to reinitialize your database, see
|

See discussion below for how to perform these actions.

Please, refer to the [Recipe: Run Gradle Tasks](https://knowledge.intershop.com#Cookbook-GradleDeveloperWorkflow(validtoGradleTools2.7)-RunGradleTasks) on how to run these tasks. This recipe also mentions how to execute a task on multiple or all cartridges at once.
...
task builds the cartridge itself and all cartridges in your component set that depend on it. This is useful to see if Java changes cause compilation errors in depending components.
...
task on that cartridge. A report is generated per cartridge at *<COMPONENTSET>/<cartridge>/target/reports/tests/index.html*.

You may also run the Gradle `test`

task on the component set. This executes the `test`

tasks of all cartridges and generates an overall report. The report can be found at *<COMPONENTSET>/target/reports/allTests/index.html*.
...
(legacy test cases written for JUnit version <= 4) ```
```
...
To let the changes take effect locally, publish the component set and redeploy your assembly, as described in [Recipe: Make Changes Effective on the Server](https://knowledge.intershop.com#Cookbook-GradleDeveloperWorkflow(validtoGradleTools2.7)-MakeChangesEffectiveOnServer). Make sure to publish the whole system component set by running the `publish`

task on the system component set, not just the added cartridge.

The developer needs to remove the development environment when finished to free system resources.

`undeploy`

on the assembly. This first execution generates an uninstall script for the services.`undeploy`

on the assembly again. This purges your server directory.The Gradle cache located *<GRADLE_USER_HOME>/cache* currently grows indefinitely and may eventually fill the hard disk.
...
(see discussion for why this has do be done twice).```
```


`GRADLE_USER_HOME`

by running `gradle_environment.bat`
...
by running `gradle_environment.sh`
...
This command only stops daemons started with the Gradle user home specified by `GRADLE_USER_HOME`

environment variable. Due to a bug in the Gradle Tooling API ([http://issues.gradle.org/browse/GRADLE-2414](http://issues.gradle.org/browse/GRADLE-2414)) IntershopStudio will partially use the default Gradle user home at *<OS_USER_HOME>/.gradle*. That is why you have to run the command twice with different values of `GRADLE_USER_HOME`
