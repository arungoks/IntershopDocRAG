---
id: 'a55654cdcddd114a1c3695bdbf6bc14f'
title: 'Public Release Note'
url: 'https://knowledge.intershop.com/kb/index.php/Display/2814L5'
scraped_at: '2026-04-21T05:40:30.873095+00:00'
---
...
Shipping and Carriers are different in their nature. While the sales channels in ICM have shipping methods which are selectable by the customers, IOM "knows" about carriers that execute those shipping methods.

For example, in the ICM one might have two shipping methods *DHL Express* and *DHL Standard* both of which are covered by single carrier *DHL*.
...
*IOM Reverse Service* needs to be configured and activated to use the Order Return feature.
...
As with many IOM features, this one is also realized by using a managed service - *IOM Download Service* - which must be configured.

Applicable return reasons are available via REST API - see [IOM Connector 4.0 - REST API - Order Return Reasons](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1824262365/IOM+Connector+4.0+-+REST+API+-+Order+Return+Reasons).
...
IOM Connector release package is available via Intershop's public Artifactory server. To add it to your release you must:

The IOM Connector contains parts that are distributed as binary artifacts (see `iomConnectorCartridges`

in the code example of section [Configure the Assembly](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1823700412/IOM+Connector+4.0+-+Setup#IOMConnector4.0-Setup-ConfiguretheAssembly)) and parts that are distributed as source (see `productionCartridges`

in the code example of section [Configure the Assembly](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1823700412/IOM+Connector+4.0+-+Setup#IOMConnector4.0-Setup-ConfiguretheAssembly)), so they can be copied and modified easily in a project.

You can retrieve the source cartridges from your repository server under com.intershop.public.source.f_iomconnector.

Unzip and copy the Multiproject contents to your project directory <ProjectHome>/projects/intershop-project /<myCustomComponentset> .
...
Edit the gradle.properties in your Multiproject and add the following:

# dependency versions ... version.com.intershop.services.iomconnector.ac_iomconnector_common = 4.0.0 version.com.intershop.services.iomconnector.ac_iomconnector_order = 4.0.0 version.com.intershop.services.iomconnector.ac_iomconnector_inventory = 4.0.0 version.com.intershop.services.iomconnector.init_iomconnector = 4.0.0 ...
...
.Change build.gradle of `ac_iomconnector_orderhistory`
...
Change build.gradle of `as_iomconnector`
...
Change build.gradle of `demo_iomconnector`
...
In order to deploy the IOM Connector with the Intershop Commerce Management, you do not have to configure anything.
...
Additionally, in order to get the IOM working with the ICM 7.8 demo scenario (inSPIRED storefront) specific demo content needs to be uploaded into the IOM. The description and the content can be found here: [Guide - IOM Demo Data Setup for IOM Connector 2.0, 3.0 and 4.0](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1821704669/Guide+-+IOM+Demo+Data+Setup+for+IOM+Connector+2.0%2C+3.0+and+4.0)
...
The web shop (storefront application) of the Intershop Commerce Management now acts as a proxy for orders' documents. This means, even if the Intershop Order Management is hosted in a private network the a customer will still be able to download the order documents.

The order document data is now available via REST API - [IOM Connector 4.0 - REST API - Get List of Documents of the Order](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1823700430/IOM+Connector+4.0+-+REST+API+-+Get+List+of+Documents+of+the+Order) and [IOM Connector 4.0 - REST API - Get Document Information](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1823700685/IOM+Connector+4.0+-+REST+API+-+Get+Document+Information).

And so does order return reasons - [IOM Connector 4.0 - REST API - Order Return Reasons](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1824262365/IOM+Connector+4.0+-+REST+API+-+Order+Return+Reasons).

IOM Connector's storefront cartridge has less development artifacts now.
