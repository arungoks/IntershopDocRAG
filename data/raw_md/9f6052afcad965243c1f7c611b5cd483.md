---
id: '9f6052afcad965243c1f7c611b5cd483'
title: 'Concept'
url: 'https://knowledge.intershop.com/kb/index.php/Display/2548T2'
scraped_at: '2026-04-21T05:37:02.973394+00:00'
---
...
Have a look at [Cookbook - Pricing (valid to 7.10)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1826517595) for related common questions.
...
To switch to JDBC/Batch import, see [Cookbook - Pricing (valid to 7.10) | Recipe - Change Price List Import Strategy (JDBC/Batch vs. ORM)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1826517595/Cookbook+-+Pricing+valid+to+7.10#EnablePriceListJDBCImport) for further details.
...
With ICM 7.10.38.14-LTS the PriceList JDBC/Batch Import only supports import mode *INITIAL *and *DELETE*. From ICM 7.10.41.0 on, the import modes *UPDATE* and *REPLACE* are available which makes the JDBC/Batch import complete.
...
Intershop supports price display formatting setup, which is not part of the pricing, but part of the [configuration framework](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1755483166).
...
This section is about handling the product price based on date/time & customer segments. Currently, it is used in the storefront when previewed from the Commerce Management application. The product price is calculated upon a given date/time (current or fixed) and on selected customer segments. The site preview can be configured with different parameters. The product price handling in preview mode is modified to respect this configuration. The technical configuration used for determining the correct price is not directly dependent on the date configured by the user in the preview configuration panel. Instead, an extension point is provided as the necessary connection between the configurations.

The **IS 7** version determines the current product price depending on the **system or selected date/time & selected customer segments**. If the storefront is called from the Commerce Management application, the price list is calculated upon the given configuration.
...
The customer segment configuration determines customer segments, specified by the user, used for the preview. Below you can see the configuration panel used for storefront preview for customer segments selection.

The Customer Segments listed in this panel appear from all enabled Customer Segmentation services (if there are no such services enabled, it should remain empty).

`PriceCalculationConfigurationProvider`

- provides the price calculation configuration values.

`DefaultPriceCalculationConfigurationProvider`

- class implementing `PriceCalculationConfigurationProvider`

. Defines an extension point for the `priceCalculationOnlineDate`

configuration parameter. The `getPriceCalculationOnlineDate`

method goes through the extensions and returns the value from the extension with highest priority and whose `isPriceCalculationOnlineDateActive`

method returns true. Otherwise, the current date is returned. The `isPriceCalculationOnlineDateParameterActive`

method returns true if an extension with an active `PriceCalculationOnlineDate`

was found, otherwise it returns false.

`PriceCalculationOnlineDateConfiguration`
