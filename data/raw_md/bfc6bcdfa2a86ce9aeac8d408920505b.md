---
id: 'bfc6bcdfa2a86ce9aeac8d408920505b'
title: 'Concept - Intershop PWA'
url: 'https://knowledge.intershop.com/kb/index.php/Display/30X852'
scraped_at: '2026-04-21T05:30:17.159257+00:00'
---
...
ICM must be run with [Secure URL-only Server Configuration](https://knowledge.intershop.com/oh/index.php/Display/40633N), which can be done by adding `SecureAccessOnly=true`
...
cookie must be enabled, so that users and baskets are synchronized between PWA and ICM.
...
For Kubernetes deployments of the PWA and the ICM in the same cluster, it is necessary to set a `SSR_HYBRID_BACKEND`

URL to the internal service URL of the ICM in addition to the `ICM_BASE_URL`

.

This URL is used for server-side requests from the PWA to the Responsive Starter Store of the ICM.

Using the external URL defined in `ICM_BASE_URL`

does not work in joint Kubernetes deployments.
...
To solve this, set the `HYBRID_FORWARDED_PORT`

as an environment variable of the NGINX container to the external port used to access the PWA.
...
To prevent issues with switching between the different ICM applications for PWA and Responsive Starter Store, it is recommended to explicitly set the `multiChannel`

> `channel`

and `application`

configuration in the PWA NGINX container.
...
See [SSR Startup - Development](https://knowledge.intershop.com/kb/go.php/g/intershop/intershop-pwa/blob/develop/docs/guides/ssr-startup.md#development) for a reference on how to achieve this locally.
...
if the development ICM is deployed with an insecure certificate or a certificate that does not work for the PWA parts.
...
can use [named capture groups](https://2ality.com/2017/05/regexp-named-capture-groups.html#replace()-and-named-capture-groups).
...
are used in the client application where [named capture groups are not yet supported by all browsers](https://github.com/tc39/proposal-regexp-named-groups#implementations).

Note

Set the `handledBy`

configuration for *Login* and *My Account* to the same value to avoid context switches between PWA and ICM during the login process.
...
access and NGINX featuresFor a more realistic Hybrid Approach environment, do not use `localhost`
...
must be set`LOGLEVEL=info`

and `LOGFORMAT=text`

help with analysis and debuggingNote
