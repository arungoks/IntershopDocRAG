---
id: '702500853f2008101ecfe58a7f8a4bac'
title: 'Public Release Note'
url: 'https://knowledge.intershop.com/kb/index.php/Display/293D39'
scraped_at: '2026-04-21T05:37:57.964892+00:00'
---
...
|---|---|---|---|
| Version | 6.3 | 7.10.17.2 - 7.10.30.* | 2.15+ |
...
This feature is governed by the *IOM Order State Service. *Once this service is configured and activated, the order status information in *My Account | Order History *is provided by the IOM in real time.
...
in the code example of the section `iomConnectorSourceCartridges`

and `iomConnectorSourceInitCartridges`

in the code example), so they can be copied and modified easily in a project.Several IOM connector cartridges are delivered as source files. These need to be included in the project's component set and adapted manually to work in the project context:
...
assembly { // ... cartridges { // ... def iomConnectorCartridges = [ 'ac_iomconnector_common', 'ac_iomconnector_order', 'ac_iomconnector_inventory', 'ac_iomconnector_rma' ] include (*(iomConnectorCartridges.collect { "com.intershop.services.iomconnector:$it" }), in: [development, test, production]) def iomConnectorSourceCartridges = [ // These are the cartridges which are delivered as source code. 'ac_iomconnector_orderhistory', 'app_sf_responsive_carriertracking', 'as_iomconnector' ] include(*(iomConnectorSourceCartridges.collect { project(":$it") }), in:[development, test, production]) def iomConnectorInitCartridges = [ 'init_iomconnector' ] include (*(iomConnectorInitCartridges.collect { "com.intershop.services.iomconnector:$it" }), in: init) def iomConnectorSourceInitCartridges = [ // This is an initialization cartridge which is delivered as source code and which contains demo data for DBInit. 'demo_iomconnector' ] include (*(iomConnectorSourceInitCartridges.collect { project(":$it") }), in: init) order = listFromAssembly('<base-assembly>') + /* ... */ + iomConnectorCartridges + iomConnectorSourceCartridges + iomConnectorInitCartridges + iomConnectorSourceInitCartridges } }

In the same *build.gradle *file add the following cartridges to `assemblyBuild.database`

:

assemblyBuild { database { inherit('<base-assembly>') initCartridges = [ // ... 'ac_iomconnector_common', 'ac_iomconnector_order', 'demo_iomconnector', 'init_iomconnector' ] } }
...
dbinit -t=ac_iomconnector_common,ac_iomconnector_order,demo_iomconnector,init_iomconnector
