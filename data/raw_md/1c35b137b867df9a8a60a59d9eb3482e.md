---
id: '1c35b137b867df9a8a60a59d9eb3482e'
title: 'Table of Contents'
url: 'https://knowledge.intershop.com/kb/index.php/Display/2Y9665'
scraped_at: '2026-04-21T05:36:36.633292+00:00'
---
...
Any HTTP proxy that meets the requirements listed in the [Architecture Overview](https://intershop.atlassian.net/wiki/pages/resumedraft.action?draftId=1917323740#IOMTechnicalOverview-Components(3.0-4.7)-ArchitectureOverview) can be used. The section [Ingress and Ingress Controller](https://knowledge.intershop.com#HelmCharts--Ingress) provides more information about actual implementation.
...
IOM requires a PostgreSQL Database Server. For further information see [https://www.postgresql.org](https://www.postgresql.org). The section [PostgreSQL Database Server](https://knowledge.intershop.com#HelmCharts--DatabaseServer) will give more information on how to use the PostgreSQL Database Server along with IOM.

An SMTP server is required to send different kinds of e-mails. The section [SMTP Server](https://knowledge.intershop.com#HelmCharts--SMTPServer) provides information on how to use the SMTP server along with IOM.
...
For more information on how to use Helm Charts for IOM see [Operate Intershop Order Management (GitHub)](https://github.com/intershop/helm-charts/tree/main/charts/iom).

The main goal of IOM Helm Charts is to provide a running IOM system. For this reason, the Helm Charts for IOM has to cover all components which are required to operate IOM, as described in the [section above](https://knowledge.intershop.com#OverallArchitecture--Overview). These are:

These components have to be translated into the world of Kubernetes.

Within a *Kubernetes* environment, an * Ingress-*object is defining how HTTP access to the underlying service (IOM in this case) has to be made. In fact, the
...
The following table shows use cases and the corresponding settings of values to properly control Helm Charts for IOM. See [Operate Intershop Order Management (GitHub)](https://github.com/intershop/helm-charts/tree/main/charts/iom) for examples.
...
*nginx.enabled* and *nginx.proxy.enabled* in the headers of the table above are two parameters that are used to control integrated *NGINX Ingress Controller* of IOM Helm Charts. These parameters are explained in detail in [Operate Intershop Order Management (GitHub)](https://github.com/intershop/helm-charts/tree/main/charts/iom) *ingress.className* in the header of the table is a parameter that has to be set at Ingress object definitions. The description of this parameter is also covered by [Operate Intershop Order Management (GitHub)](https://github.com/intershop/helm-charts/tree/main/charts/iom).

According to the information in [Overall Architecture](https://knowledge.intershop.com#OverallArchitecture--Overview) described above, two different types of IOM application servers exist: one with IOM singleton and scalable applications, which has to exist in each installation of IOM exactly once, and 0 or more application servers, which are running IOM scalable applications only.

The IOM Helm Charts are using a [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/) to realize the required behavior. Within a stateful set, the pods have fixed names, all ending with a number. The first pod (ending with 0) is the one and only, which is running IOM singleton applications.

Configuration examples and a full list of configuration parameters can be found in [Operate Intershop Order Management (GitHub)](https://github.com/intershop/helm-charts/tree/main/charts/iom).
...
The SMTP server, which is part of IOM Helm Charts is dedicated for test, demo and preproduction installations only. [MailHog](https://github.com/mailhog/MailHog) is such a kind of SMTP server, which was created especially for testing purposes. For this reason, MailHog is part of IOM Helm Charts.

MailHog is not an SMTP server only. It also provides a REST API and a web-GUI to get access to the received e-mails. Helm Charts for IOM provides the required Ingress object, which has to be configured in the same way as described in section [Ingress and Ingress controller](https://knowledge.intershop.com#HelmCharts--Ingress).
...
All these data have to be provided by implementation partners in a very special way, which is described in [Guide - IOM Standard Project Structure 3.X](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1882787535/Guide+-+IOM+Standard+Project+Structure+3.X). The easiest way to provide artifacts in this standard project structure is the usage of the [IOM Project Archetype](https://github.com/intershop/iom-project-archetype). Projects created this way also provide the ability to easily create the project-specific IOM docker image. This docker image can then be rolled out by IOM Helm Charts.
...
| Key | Description |
|---|---|
| tenant | The name of the tenant, e.g. Intershop Note
|
| environment | The name of the environment, e.g. prod, pre-prod, etc. Note
|
...
Please see section [Health Check Requests](https://knowledge.intershop.com#Guide-IntershopOrderManagement-TechnicalOverview(3.0-4.7)-HA--HealthCheckRequests) for more information.
...
For high availability, the Helm value *persistence.storageClass* has to be set to a highly available storage class. See [Operate Intershop Order Management (GitHub)](https://github.com/intershop/helm-charts/tree/main/charts/iom).

According to section * Ingress and Ingress Controller* the

`ingress-nginx.controller.replicaCount`

, see Session stickiness and session failover are realized by the *Ingress controller*. Depending on the capabilities of the *Ingress controller*, this is realized by the global or by the integrated *NGINX Ingress controller*, see section * Ingress and Ingress Controller*.
