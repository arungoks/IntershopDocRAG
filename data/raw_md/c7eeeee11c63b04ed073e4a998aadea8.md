---
id: 'c7eeeee11c63b04ed073e4a998aadea8'
title: 'Cookbook'
url: 'https://knowledge.intershop.com/kb/index.php/Display/2Y5600'
scraped_at: '2026-04-21T05:39:53.892381+00:00'
---
...
How to work with an existing (and therefore configured) assembly as part of the [developer workflow](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1789069382). This includes execution of the assembly process.
...
An assembly can also be created with the scripts in the development-setup of an Intershop delivery package (See [Cookbook - Setup CI Infrastructure](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1822571078/Cookbook+-+Setup+CI+Infrastructure)).
...
To exclude the artifact from inheritance, see [Recipe: Customize Assembly Inheritance](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-CustomizeAssemblyInheritance).
...
See subsection [Order from Groovy List Literal](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-OrderFromGroovyList) for how to specify both included cartridges and their order without redundancies.
...
The same consideration like in the recipe [Cookbook - Gradle Assembly Tools (valid to 7.8)#Add Cartridges to an Assembly](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-AddCartridgestoanAssembly) regarding transitive vs. intransitive dependencies applies here.
...
The new environment initially contains no cartridges. Refer to the recipe [Cookbook - Gradle Assembly Tools (valid to 7.8)#Add Cartridges to an Assembly](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-AddCartridgestoanAssembly) for adding cartridges to this environment.

Example: Add a new environment to your assembly that inherits all the cartridges from the production environment.

Solution: Add the environment `prelive`

to your assembly *build.gradle*.
...
The new host type initially contains no components. Refer to the recipe [Cookbook - Gradle Assembly Tools (valid to 7.8)#Add Infrastructure Components to an Assembly](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-AddInfrastructureComponentstoanAssembly) for adding components to this environment.
...
When including cartridges transitively (see [Recipe: Add Cartridges to an Assembly](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-AddCartridgestoanAssembly)), the depending libraries become system components themselves to the assembly. Therefore dealing with libraries equals the dealing with any other system component.
...
You can inspect the origin of different dependencies as described below and can use [Transitive Dependency Management](https://knowledge.intershop.com#Cookbook-GradleAssemblyTools(validto7.8)-TransitiveDependencyManagement) to exclude unwanted dependencies to resolve collisions.
...
For this to work you have to find an assembly, that contains a suitable dump (e.g., containing a subset of the cartridges in your assembly). This is typically be the same assembly that you inherit dependencies from.
...
This activates packaging of branding installations after generation of the dump and publishes them as an artifact *branding* of type *share*. The package will be automatically deployed by standard Intershop 7 build plugins.
...
To use Solr-based search in the Commerce Management application and storefront, it is necessary to deploy the Solr search service and a Solr server.

`com.intershop.services:ac_search_solr`

to your assembly (follow the recipe See discussion for details.

The Intershop 7 delivery comes with a component `3rd_solr_war_int`

that contains the Solr *war*-file and a deployment script.

The deployment script of `3rd_solr_war_int`

registers the Solr *war*-file in the Tomcat of the Intershop 7 appserver instance `appserver0`

.

To deploy this only in some scenarios, create a new host-type `appserverWithSolr`

. Add the following components to the host-type `appserverWithSolr`

:

`com.intershop.services:3rd_solr_war_int`

`appserver`

To do so use the following code in the *build.gradle* file of your assembly:

... assembly { ... // Before your existing inheritFrom-block hostTypes { appserverWithSolr { includeLocal = true include('com.intershop.services:3rd_solr_war_int') { transitive = false } } } // In your existing inheritFrom-block inheritFrom('<org>:<name>') { ... eachDependency { dependency -> // // include all components in host type 'appserver' also in host type 'appserverWithSolr' if (dependency.configurations.contains(appserver)) { dependency.configurations << appserverWithSolr } } } }

Single machine deployments (host type `all`

) include all host types including `appserverWithSolr`

. This way the Solr *war*-file is deployed into the Apache Tomcat server of an Intershop 7 application server.

When deploying in a cluster, simply use the host type `appserver`

instead of `appserverWithSolr`

to avoid deployment of the Solr *war*-file. Deploy your Solr server manually (e.g., without the Intershop Deployment Tools) using a standard Apache Tomcat and Solr *war*-file.

Intershop 7.5 introduces a new assembly `intershop7-solr`

. This assembly contains Solr search functionality in addition to the `intershop7`

base components.

This assembly defines the additional host type `solr`

, that deploys a stand-alone Solr search server inside a Tomcat servlet container. Furthermore, host type `all`

also contains Solr search, which will install a Solr server inside the first appserver's Tomcat by default.

`ac_search_solr`

to Connect to Solr ServerProvide a custom *deploy.gradle* for your assembly. In this *deploy.gradle* file add the following code:

if (deploymentBootstrap.hostType.hostType.name == 'all') { apply plugin: com.intershop.deploy.intershop.SolrDeploymentPlugin }

The `SolrDeploymentPlugin`

configures *<IS_SHARE>/system/config/cartridges/ac_search_solr.properties* to connect to the Solr running the Apache Tomcat of Intershop 7. It is only applied for single machine deployments.

For clustered deployments, configure the connection to the separate Solr server during deployment in your *settings.gradle*. See recipe [Configure Intershop 7](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1783302208#Cookbook-GradleDeploymentTools(7.4CI-ICM7.7)-ConfigureIntershop7) in the [Cookbook - Gradle Deployment Tools (7.4 CI - ICM 7.7)](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1783302208) on how to specify properties read by the configuration framework.

Properties relevant for the Solr based search service are:

| Property | Description | Example | Default Value |
|---|---|---|---|
`solr.SolrServerURL` | Server URL of the Solr Server including Protocol, Host and Port | ```
http://localhost:8983
``` | URL of external Tomcat of application server |
`solr.SolrClusterNodeURLs` | Server URLs for the all Solr Server Nodes which belong to Solr Cluster including Protocol, Host and Port separated by semicolon | ```
http://solrhost1:8983;
``` | `${solr.SolrServerURL}` |
`solr.SolrServerContextPath` | The Context Path where the Solr Server Web Application is installed at the Server | `/solr` | `/solr` |

Gradle Tools 2.0 introduces a new deployment extension for configuring the Solr cluster for Intershop 7, see [Cookbook - Gradle Deployment Tools](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1783302208#Cookbook-GradleDeploymentTools(7.4CI-ICM7.7)-Recipe-Configure-the-Deployment).

Make sure, that `com.intershop.deploy.intershop.SolrDeploymentPlugin`

is being applied during deployment of the assembly project by either:

`deploy.gradle`

script from `intershop7-solr`

Adding the following line to your custom assembly deployment script:
