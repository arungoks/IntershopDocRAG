---
id: 'c84fce99dcaf715387b80cb30fb459b7'
title: 'Public Release Note'
url: 'https://knowledge.intershop.com/kb/index.php/Display/B29919'
scraped_at: '2026-04-21T05:37:01.934914+00:00'
---
...
|---|---|---|---|
| Version | 7.0 | 7.10.31.2+ | 3.1+ |

This version has the same range of features as the [IOM Connector 6.5](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1938753962/Public+Release+Note+-+IOM+Connector+6.5), but it offers support for Tomcat 9 and the updated dependencies.
...
| Service | End Point | IOM REST API | Note |
|---|---|---|---|
...
The IOM Connector provides instruments that route any order placed from ICM to IOM. To use this feature, you must configure the order export (see [Configure Order Export](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1821711350#Guide-ICMandIOMConfigurationTasks(validto2.17)-ConfigureOrderExport)), which depends on an existing service configuration (see [Configure IOM Order Service](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1821711350#Guide-ICMandIOMConfigurationTasks(validto2.17)-ConfigureIOMOrderService)). As mentioned before, you may tweak the export settings on the configuration page, but we generally use *on order creation* as trigger.
...
Once a return request has been approved and the return slip has been generated in the IOM or in the *My Account* section, it can be downloaded from the link displayed on the *Order History* page of the order.
...
This feature allows to display attached documents (notes/information) on the *Order Details* page. The *Order Document* retrieval from IOM has a cache handling mechanism that preserves the information for a given time interval. ICM's *Order Details* page rendering process requests IOM to report the number of documents attached to the specific order. The badge to the right of the *View All* link indicates the number of documents. Click *View All* to display a list of all documents. The list shows information such as note type (invoice, credit , etc.), note creation date/time, and a link for the downloading the document.
...
directive, just add the *excludes* listed below to it.
...
For details on how to deploy an assembly, see [Recipe: Run the Deployment (Initial Installation / Upgrade / Downgrade).](https://knowledge.intershop.com/kb/go.php/a/ENFDEVDOC/pages/1848547085/Cookbook+-+Deployment+Tools+ICM+7.10#Cookbook-DeploymentToolsICM7.10-Run-the-Deployment)
...
Additionally, in order to get the IOM working with the ICM 7.10 demo scenario (inSPIRED storefront), specific demo content needs to be uploaded into IOM. For information on how to do this, please refer to the IOM documentation.
